...
var Slices = require('string-slices-array-push/es5'); // ranges managing library
var replaceSlicesArray = require('string-replace-slices-array/es5');
...
var doDecodeBRs = util.doDecodeBRs,
    ...
    doConvertDashes = util.doConvertDashes,
    doInterpretErroneousNBSP = util.doInterpretErroneousNBSP,
    fixMissingAmpsAndSemicols = util.fixMissingAmpsAndSemicols,
    defaultsObj = util.defaultsObj,
    isUppercaseLetter = util.isUppercaseLetter,
    isLowercaseLetter = util.isLowercaseLetter,
...
function detergent(inputStr, inputOpts) {
  ...
  if (arguments.length > 1 && !isObj(inputOpts)) {
    throw new Error('detergent(): [THROW_ERROR_ID01] Options object must be a plain object, not ' + (typeof inputOpts === 'undefined' ? 'undefined' : _typeof(inputOpts)));
  }
  // defaults object (`defaultsObj`) is in util.js to keep it DRY
  // fill any settings with defaults if missing:
  var o = Object.assign({}, defaultsObj, inputOpts);
  ...
  var lastIndexOfTheLastOfDotsInRow = null;
  ...
  for (i = 0, len = str.length; i < len; i++) {
    ...
    if (numCode < 32 && allOK) {
      ...
      //
      if (str[i + 2] !== undefined && str[i + 1] + str[i + 2] === '..' && (lastIndexOfTheLastOfDotsInRow === null || lastIndexOfTheLastOfDotsInRow < i)) {
      ...
      if ((str[i - 1] === ' ' || str[i - 1] === '\xa0' || str[i - 1] === '\u200A') && str[i + 1] !== ' ') {
      ...
      if ((str[i - 1] === '\xa0' || str[i - 1] === ' ') && str[i + 1] !== '$' && str[i + 1] !== '£' && str[i + 1] !== '€' && str[i + 1] !== '₽' && str[i + 1] !== '0' && str[i + 1] !== '1' && str[i + 1] !== '2' && str[i + 1] !== '3' && str[i + 1] !== '4' && str[i + 1] !== '5' && str[i + 1] !== '6' && str[i + 1] !== '7' && str[i + 1] !== '8' && str[i + 1] !== '9') {
        // add space after it:
        rangesArr.add(i + 1, i + 1, ' ');
      }
      ...
      if (str[i + 1] + str[i + 2] + str[i + 3] === '/ul' || str[i + 1] + str[i + 2] === 'li') {
      ...
      if (scriptTagStarts !== null && str[i + 1] + str[i + 2] + str[i + 3] + str[i + 4] + str[i + 5] + str[i + 6] === 'script') {
        allOK = true;
        if (str[i + 7] === '>') {
    ...
    if (str[i] === '.') {
      ...
      if (first + second !== 'js' && nextThreeChars !== 'jpg' && nextThreeChars !== 'png' && nextThreeChars !== 'gif' && nextThreeChars !== 'svg' && nextThreeChars !== 'htm' && nextThreeChars !== 'pdf' && nextThreeChars !== 'psd' && nextThreeChars !== 'tar' && nextThreeChars !== 'zip' && nextThreeChars !== 'rar' && nextThreeChars !== 'otf' && nextThreeChars !== 'ttf' && nextThreeChars !== 'eot' && nextThreeChars !== 'php' && nextThreeChars !== 'rss' && nextThreeChars !== 'asp' && nextThreeChars !== 'ppt' && nextThreeChars !== 'doc' && nextThreeChars !== 'txt' && nextThreeChars !== 'rtf' && nextThreeChars !== 'git' && nextThreeChars + fourth !== 'jpeg' && nextThreeChars + fourth !== 'html' && nextThreeChars + fourth !== 'woff') {
        ...
        !onUrlCurrently && isUppercaseLetter(str[i + 1]) || onUrlCurrently && isLetter(str[i + 1]) && isUppercaseLetter(str[i + 1]) && isLetter(str[i + 2]) && isLowercaseLetter(str[i + 2])) && str[i + 1] !== ' ' && str[i + 1] !== '.' && str[i + 1] !== '\n') {
          rangesArr.add(i + 1, i + 1, ' ');
        }

        // 2. REMOVING SPACES BEFORE IT:
        if (str[i - 1] !== undefined && str[i - 1].trim() === '' && str[i + 1] !== '.' && (str[i - 2] === undefined || str[i - 2] !== '.') // that's for cases: "aaa. . " < observe second dot.
        ) {
            // march backwards
            for (y = i - 1; y--;) {
              if (str[y].trim() !== '') {
      ...
      if (o.addMissingSpaces && str[i + 1] !== undefined && !onUrlCurrently && !isNumeric(str[i + 1]) && str[i + 1] !== ' ' && str[i + 1] !== '\n') {
        // comma, not on URL, not followed by number = add space afterwards
        rangesArr.add(i + 1, i + 1, ' ');
      }
    } else if (str[i] === ';') {
      // -----
      if (o.addMissingSpaces && str[i + 1] !== undefined && !onUrlCurrently && str[i + 1] !== ' ' && str[i + 1] !== '\n' && str[i + 1] !== '&' && str[i + 1] !== '\xa0' && allOK) {
        rangesArr.add(i + 1, i + 1, ' ');
      }
      // -----
    }
  ...
  if (o.convertEntities) {
    ...
    for (y = numerics.length; y--;) {
      // loop backwards for better efficiency
